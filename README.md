...
├── environment.yml              # python environment used in analysis
└── README.md
```
...
1. Create and activate the conda environment:
   ...
   conda activate warhammer_env
   ```
2. Create a folder called `data` in the project root directory
3. Download the four CSV files listed above and place them in the `data/` folder
4. Run `data_cleaning.ipynb` from top to bottom
   - Cleans the raw data, exports parquet files to `outputs/`, and creates `wh_database.duckdb` in the project root
5. Open `viz.R` in RStudio and run the script (`Ctrl+Shift+Enter`)
